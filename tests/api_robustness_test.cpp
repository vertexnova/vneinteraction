...
TEST(ApiRobustness, OrbitTrackballBehaviorIsEnabledDefaultTrue) {
    vne::interaction::OrbitTrackballBehavior b;
    EXPECT_TRUE(b.isEnabled());
}

TEST(ApiRobustness, OrbitTrackballBehaviorSetEnabledFalseReflected) {
    vne::interaction::OrbitTrackballBehavior b;
    b.setEnabled(false);
    EXPECT_FALSE(b.isEnabled());
}

TEST(ApiRobustness, DisabledOrbitTrackballBehaviorDoesNotMoveCamera) {
...
TEST(ApiRobustness, InspectControllerScrollZoom) {
    ...
    ctrl.setCamera(cam);
    ctrl.onResize(1280.0f, 720.0f);
    ctrl.orbitTrackballBehavior().setZoomSpeed(2.0f);

    const float dist_before = ctrl.orbitTrackballBehavior().getOrbitDistance();
    ...
    const float dist_after = ctrl.orbitTrackballBehavior().getOrbitDistance();
...
TEST(ApiRobustness, FitToAABBPositionsCOI) {
    ...
    const auto coi = ctrl.orbitTrackballBehavior().getCenterOfInterestWorld();
