...
- **Behaviors**: `OrbitBehavior`, `FreeLookBehavior`, `OrthoPanZoomBehavior`, `TrackBehavior` — each implements `ICameraBehavior` and works with `vne::scene::ICamera` (perspective or orthographic).
...
- **Behaviors**: `OrbitBehavior`, `FreeLookBehavior`, `OrthoPanZoomBehavior`, `TrackBehavior` — rotation modes (Euler/Quaternion), pivot modes, zoom methods, inertia, fit-to-AABB.
