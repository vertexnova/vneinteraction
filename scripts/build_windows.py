...
def interactive_mode(config: BuildConfig):
    print("=== VneInteraction Interactive Build Configuration ===")
...
def build_cmake_command(project_root: Path, build_type: str, lib_type: str) -> List[str]:
    return [
        ...
        "-DVNE_INTERACTION_LIB_TYPE=" + lib_type, "-DVNE_INTERACTION_TESTS=ON",
        str(project_root),
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Build VneInteraction for Windows")
    parser.add_argument("-t", "--build-type", choices=["Debug", "Release", "RelWithDebInfo", "MinSizeRel"], default="Debug")
    parser.add_argument("-l", "--lib-type", choices=["shared", "static"], default="shared", help="Library type (shared or static)")
    ...
    build_dir = project_root / "build" / config.lib_type / config.build_type / f"build-windows-{config.compiler}-{compiler_version}"
