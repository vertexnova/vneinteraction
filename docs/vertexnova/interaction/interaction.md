...
- **Public API** (`include/vertexnova/interaction/`): `camera_behavior.h`, `camera_rig.h`, `orbit_trackball_behavior.h`, `free_look_behavior.h`, `ortho_pan_zoom_behavior.h`, `follow_behavior.h`, `input_mapper.h`, `inspect_controller.h`, `navigation_3d_controller.h`, `ortho_2d_controller.h`, `follow_controller.h`, `interaction_types.h`, `interaction.h` (umbrella), `version.h`.
...
- **orbit_trackball_behavior.h**: `OrbitTrackballBehavior` — orbit around a center of interest; Euler or Quaternion rotation; pivot modes (COI, ViewCenter, Fixed); pan, zoom, inertia, fitToAABB.
...
- **inspect_controller.h**: `InspectController` — 3D inspection (medical, CAD); wraps `OrbitTrackballBehavior` + `InputMapper` with orbit preset; pivot, DOF toggles, fitToAABB.
- **navigation_3d_controller.h**: `Navigation3DController` — 3D environment traversal; FPS, Fly, or Game mode; wraps `FreeLookBehavior` (+ optional `OrbitTrackballBehavior` in Game mode) + `InputMapper`.
